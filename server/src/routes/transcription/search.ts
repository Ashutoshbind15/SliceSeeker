...
import { z } from "zod";
import { searchTranscripts } from "../../services/transcription/search.js";
...
export const searchTranscriptsHandler = async (req: Request, res: Response) => {
  ...
  try {
    const results = await searchTranscripts({ ...parsed.data, limit });
    res.json({
