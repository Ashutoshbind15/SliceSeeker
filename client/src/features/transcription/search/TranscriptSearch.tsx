...
import { Mic, Search, SlidersHorizontal, PlayCircle } from "lucide-react";
...
const TranscriptSearch = () => {
  ...
  const searchQuery = useTranscriptSearchResultsQuery(
    submittedSearch,
  ...
  return (
    <PageShell
      title="Speech search"
      help={
        <PageHelp title="About speech search">
          <p>
            Search spoken content across Whisper transcript segments and jump to
            the matching moment in the source video.
          </p>
        </PageHelp>
      }
    >
              ...
              <div className="grid gap-4 sm:grid-cols-3 pt-4 border-t border-border/50 animate-in fade-in slide-in-from-top-2">
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      <FieldLabel
                        htmlFor="transcript-collection-filter"
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      <FieldLabel
                        htmlFor="transcript-upload-filter"
                        className="text-xs uppercase tracking-wider text-muted-foreground"
                      >
                        Video
                      </FieldLabel>
                      ...
                      >
                        <SelectTrigger
                          id="transcript-upload-filter"
                          className="w-full bg-background rounded-xl"
                          aria-invalid={fieldState.invalid}
                        >
                          <SelectValue placeholder="All transcribed videos" />
                        </SelectTrigger>
                        <SelectContent>
                ...
                <Controller
                  ...
                  render={({ field, fieldState }) => (
                    <Field data-invalid={fieldState.invalid}>
                      <FieldLabel
                        htmlFor="transcript-result-limit"
                        className="text-xs uppercase tracking-wider text-muted-foreground"
                      >
                        Max Results
                      </FieldLabel>
      ...
      <div className="min-h-[200px]">
        ...
        searchableUploads.length === 0 ? (
          <QueryEmptyState
            icon={<Mic />}
            title="No transcribed videos yet"
            description="Transcribe a video first, then return here to search spoken content."
        ...
        {uploadsFetchError ? (
          <QueryErrorAlert
            ...
            title="Could not load transcribed videos"
        ...
        results.length === 0 ? (
          <QueryEmptyState
            icon={<Search />}
            title="No matching speech segments"
            description={`No results found for "${submittedSearch?.query}". Try a different phrasing.`}
            className="rounded-2xl border bg-muted/30"
          />
        ) : null}
