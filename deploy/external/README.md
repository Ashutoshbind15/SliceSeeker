...
| `ENQUEUE_CONCURRENCY` | 1 | Max concurrent BullMQ adds + DB writes when fan-out enqueueing jobs for one file (default `16`) |
...
- **Schema**: run `docker-compose.init.yml` after upgrades or on empty databases. Application `/ready` endpoints verify `AI_GATEWAY_API_KEY` and schema but do not migrate.
