...
2. **Upload** a short video via the upload screen (tus → object storage).
3. Pick one modality and start processing:
...
5. Open that modality's **search** page and try a natural-language query.

Each modality has its own process, search, and costs screens. Indexing one does not index the others.
...
See [Search](/docs/search) for transcript and frame methods.
