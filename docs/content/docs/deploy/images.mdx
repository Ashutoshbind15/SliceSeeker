...
Release tags on GHCR match the root `package.json` version (for example `0.1.2`). Each release also publishes `:latest` and `:sha-<short>` tags. Prefer the version tag in production.
...
```bash
INDEXER_API_IMAGE=ghcr.io/ashutoshbind15/sliceseeker-indexer-api:0.1.2
INDEXER_WORKER_IMAGE=ghcr.io/ashutoshbind15/sliceseeker-indexer-worker:0.1.2
ADMIN_UI_IMAGE=ghcr.io/ashutoshbind15/sliceseeker-admin-ui:0.1.2
SEARCH_API_IMAGE=ghcr.io/ashutoshbind15/sliceseeker-search-api:0.1.2
DB_PUSH_IMAGE=ghcr.io/ashutoshbind15/sliceseeker-db-migrate:0.1.2
```
