...
Each modality records usage when the gateway returns cost metadata. Missing metadata fails clearly rather than inventing numbers. Use the per-modality costs screens in the admin UI for A/B on the same fixture video.
