...
Each path records usage when the gateway returns cost metadata. Missing metadata fails clearly rather than inventing numbers. The admin UI costs screens show spend by file (and mix bars on hybrid) with a selectable USD decimal precision. Use those screens for A/B on the same fixture video.

## Admin UI playback

Search and process previews play the source file. There is no separate thumbnail pipeline. Presigned playback links use `S3_PUBLIC_ENDPOINT`. That URL must be reachable from the operator's browser.
