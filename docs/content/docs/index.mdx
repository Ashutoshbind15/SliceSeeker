...
**demo-search-ai** is a self-hostable **internal** tool for semantic video search. Other backend services call Phase 2 (`search-api` / `search-client`) to seek and search video slices. Operators use the admin UI. There is no built-in login — see [Deploy](/docs/deploy) for the network-trust model.
